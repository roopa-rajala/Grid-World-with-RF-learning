...
class ExperimentThree:
    ...
    def PExploit(self):
        ...
        if r<0.15:
            ...
            self.PGreedy()
        episode += 1
        self.root.after(1, self.PExploit)
        ...
        for (pos, i, j, c, d, w) in pickDrop:
            if player[0] == i and player[1] == j:
                ...
                if c == "red" and hasblock == 0 and d > 0:
                    ...
                    self.learnSARSA(player[0], player[1], 4, 12, player[0], player[1], hasblock)
                    episode+=1
                    ...
                    pickDrop[pos] = tuple(lst)
                    self.learnSARSA(player[0], player[1], 5, 12, player[0], player[1], hasblock)
                    episode+=1
                    noOfOperatorsApplied += 1
    def PGreedy(self):
        global text,  me, episode, hasblock
            ...
            for (pos, i, j, c, d, w) in pickDrop:
                if new_x == i and new_y == j:
                    if c == "red" and hasblock==0 and d>0:
                        ...
                        flag = 2
            self.learnSARSA(player[0], player[1], direct, -1, new_x, new_y, hasblock)
            if episode==50 or episode==100 or episode ==500 or episode ==1000 or episode==1500 or episode ==2000 or episode==3000 or episode==4000 or episode==5000 or episode==6000:
                print(episode,score)
            player = (new_x, new_y)
            flag = 0
    ...
    def checkedges(self,new_x,new_y):
        if (new_x == 0 and new_y>0 and new_y<4):
            self.learnSARSA(player[0], player[1],3, -1, new_x, new_y, hasblock)
        elif(new_x == 4 and new_y>0 and new_y<4):
            self.learnSARSA(player[0], player[1], 2, -1, new_x, new_y, hasblock)
        elif(new_y == 0 and new_x>0 and new_x<4):
            self.learnSARSA(player[0], player[1],0 , -1, new_x, new_y, hasblock)
        elif(new_y == 4 and new_x>0 and new_x<4):
            self.learnSARSA(player[0], player[1], 1, -1, new_x, new_y, hasblock)
    ...
    def SARSA(self, statex, statey, action, reward, value, hasblock):
    ...
    def learnSARSA(self, statex1, statey1, direct, reward, statex2, statey2, hasblock):
        qnext = self.getQ(statex2, statey2, hasblock, direct)
        self.SARSA(statex1, statey1, direct, reward, reward + gamma * qnext, hasblock)
        # every second..
